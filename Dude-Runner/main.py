...
def player_animation():
    ...
    if player_stay_rect.bottom < 360:
        player_surf = player_jump
    else:
        player_index += 0.07
        if player_index >= len(player_walk_animation):
            player_index = 0
        player_surf = player_walk_animation[int(player_index)]


# def enemy_animation():
#     global enemy_image, ROTATE
#     ROTATE += 0.1
#     if ROTATE >= 20: ROTATE = 0
#     enemy_image = pygame.transform.rotate(enemy_image, ROTATE)
#

pygame.init()
screen = pygame.display.set_mode((1000, 499))
pygame.display.set_caption('Dude Runner')
...
player_walk = pygame.image.load('img/player_walk2.png').convert_alpha()
player_jump = pygame.image.load('img/player_jump.png').convert_alpha()
player_walk_animation = [player_stay, player_walk]
player_index = 0
player_surf = player_walk_animation[player_index]
player_stay_rect = player_surf.get_rect(topleft=(150, 320))
...
while True:
    ...
    if game_active:
        ...
        screen.blit(player_surf, player_stay_rect)
        SCORE = show_score()
        player_animation()
        # enemy_animation()

        background_rest.right -= GAME_SPEED
